...
public interface CourseRepository extends JpaRepository<Course, String> {
	@Query(value = "SELECT * FROM course WHERE unaccent(lower(title)) ILIKE unaccent(lower('%' || :title || '%'))", nativeQuery = true)
	Page<Course> searchCourseByTitle(String title, Pageable pageable);
	
	@Query(value = "SELECT * FROM course WHERE unaccent(lower(title)) ILIKE unaccent(lower('%' || :title || '%'))", nativeQuery = true)
	List<Course> searchCourseByTitle(String title);
	
	List<Course> findByRegisteredUser(User user);
